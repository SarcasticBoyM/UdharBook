  ...
  try {
    ...
    return NextResponse.json({
      ...
      setting: {
        ...
        lastCredsUpdateAt: diagnostics.lastCredsUpdateAt ?? null,
        lastCredsSaveError: diagnostics.lastCredsSaveError ?? null,
        hasRegisteredCreds: diagnostics.hasRegisteredCreds ?? null,
        socketStartedAt: diagnostics.socketStartedAt ?? null,
        qrGeneratedAt: diagnostics.qrGeneratedAt ?? null,
        lastHeartbeatAt: diagnostics.lastHeartbeatAt ?? null,
        lastHeartbeatSocketState: diagnostics.lastHeartbeatSocketState ?? null,
        postQrAliveCheckedAt: diagnostics.postQrAliveCheckedAt ?? null,
        socketState10sAfterQr: diagnostics.socketState10sAfterQr ?? null,
        socketAlive10sAfterQr: diagnostics.socketAlive10sAfterQr ?? null,
        processExitAt: diagnostics.processExitAt ?? null,
        processExitEvent: diagnostics.processExitEvent ?? null,
        lastConnectionUpdateAt: diagnostics.lastConnectionUpdateAt ?? null,
      },
      recentJobs,
    });
  } catch (error) {

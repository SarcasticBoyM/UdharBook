...
import { isAccountsRole, isRetiredTrackingRole, isSalesRole, isShopAdminRole, normalizeFixedRole } from "@/lib/operational-roles";

export function canDelete(role: UserRole | string): boolean {
  if (isRetiredTrackingRole(role)) return false;

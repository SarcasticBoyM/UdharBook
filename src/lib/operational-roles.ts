...
export function normalizeFixedRole(role: AppRole): AppRole {
  ...
  // Retired tracking roles remain recognizable for existing user records, but
  // they are no longer assignable or authorized for an application module.
  if (value === "DRIVER" || value === "SCHOOL_ADMIN" || value === "SCHOOL_DRIVER") return value;
...
export function isRetiredTrackingRole(role: AppRole) {
  const normalized = normalizeFixedRole(role);
  return normalized === "DRIVER" || normalized === "SCHOOL_ADMIN" || normalized === "SCHOOL_DRIVER";
}
